...
class _SelectCategoryScreenState extends CloudState<SelectCategoryScreen> {
  ...
  Future<void> categoryClickHandler(CategoryModel category, int index) async {
    if (!TouchManager.canProcessTouch()) return;

    LoadingWidgets.showLoader(context);

    try {
      // Validate if user has valid package for this category
      final result = await context
          .read<FetchCategoryCubit>()
          .validateCategoryForListing(categoryId: category.id!);

      LoadingWidgets.hideLoader(context);

      // If validation succeeds, proceed with navigation
      if (result.modelList.isEmpty && category.subcategoriesCount == 0) {
        // No subcategories, navigate to add item details
        ...
        // Has subcategories, navigate to nested category screen
        addCloudData("breadCrumb", [category]);
      ...
      LoadingWidgets.hideLoader(context);
      // Show error message if user doesn't have valid package
...
class SelectNestedCategory extends StatefulWidget {
  const SelectNestedCategory({super.key, required this.current});
  ...
  static Route route(RouteSettings settings) {
    ...
    return MaterialPageRoute(
      builder: (context) {
        return SelectNestedCategory(current: arguments['current']);
      },
    );
  }
...
class _SelectNestedCategoryState extends CloudState<SelectNestedCategory> {
  ...
  void _onBreadCrumbItemTap(List<CategoryModel> dataList, int index) {
      ...
      child: PopScope(
        ...
        onPopInvokedWithResult: (didPop, result) {
          ...
          if (breadCrumbData.length == 1) {
            ...
            _onBreadCrumbItemTap(breadCrumbData, breadCrumbData.length - 2);
          }
        },
        child: Scaffold(
          appBar: UiUtils.buildAppBar(
  ...
  Future<void> categoryClickHandler(
    ...
    try {
      ...
      final result = await context
          ...
          .validateCategoryForListing(categoryId: categoryModel.id!);

      LoadingWidgets.hideLoader(context);

      // If validation succeeds, proceed with navigation
      if (result.modelList.isEmpty && categoryModel.subcategoriesCount == 0) {
        // No subcategories, navigate to add item details
        screenStack++;
        Navigator.pushNamed(
          context,
        ...
        Navigator.pushNamed(
          context,
          Routes.selectNestedCategoryScreen,
          arguments: {"current": categoryModel},
        ).then((value) {
          if (value == true) {
            screenStack--;
