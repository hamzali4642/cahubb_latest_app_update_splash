...
class FilterScreenState extends State<FilterScreen> {
  static List<PostedSinceItem> _postedSince = [
  ...
  late String postedOn = widget.filter?.postedSince ?? _postedSince[0].value;
  ...
  void getCustomFieldsData() {
    ...
    if (selectedCategory != null || widget.categoryIds.isNotEmpty) {
      context.read<FetchCustomFieldsCubit>().fetchCustomFields(
        categoryIds: [?selectedCategory?.id, ...widget.categoryIds].join(','),
        isForFilter: true,
      );
    }
  }
  ...
  void setDefaultVal({bool isRefresh = true}) {
    if (isRefresh) {
      postedOn = _postedSince[0].value;
      Constant.itemFilter = null;
      searchBody[Api.postedSince] = _postedSince[0].value;
  ...
  bool checkFilterValSet() {
    if (postedOn != _postedSince[0].value ||
  ...
  Widget postedSinceOption(BuildContext context) {
    int index = _postedSince.indexWhere((item) => item.value == postedOn);
        ...
        child: Row(
          children: [
            ...
            Padding(
              padding: const EdgeInsetsDirectional.only(start: 15.0),
              child: CustomText(
                _postedSince[index].status.translate(context),
                color: context.color.textDefaultColor.withValues(alpha: 0.5),
              ),
            ),
            Spacer(),
