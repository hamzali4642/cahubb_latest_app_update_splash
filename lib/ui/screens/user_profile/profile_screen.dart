...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  Widget getProfileImage() {
    ...
    if (HiveUtils.isUserAuthenticated()) {
      if ((HiveUtils.getUserDetails().profile ?? "").isEmpty) {
        ...
        profileImageWidget = CustomImage(
          src: HiveUtils.getUserDetails().profile!,
  ...
  void proceedToDeleteProfile() async {
    try {
      ...
      if (error.code == "requires-recent-login") {
        // Check if user is logged in via phone + password (no Firebase auth)
        final userDetails = HiveUtils.getUserDetails();
        if (userDetails.type == 'phone') {
          // User logged in with phone, send OTP for verification
          _showDeleteVerificationScreen();
        } else {
          // User logged in with email or social, ask to login again
          askToLoginAgain();
        }
      } else {
  ...
  void _showDeleteVerificationScreen() async {
